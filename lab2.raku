...
sub extract_title {
	...
	for @tracks -> $track { 
		...
		##
		my @parts = $track.split('SEP>');
		my $tracktitle = @parts[*-1];
...
sub comments {
	...
	for @tracks -> $title { 
		...
		# removing starting from feat. onward
		# correction: i was incorrectly used to ignore case sensitivity, then fixed
		$_ ~~ s/ feat \. .* //;

		# (   [   {   \   /  _   -   :   "   `   +   =   feat.
	
		# Repeat for the other symbols
		# Removing symbols and using ascii literals to avoid char probs
		$_ ~~ s/ \x28 .*//; # (
		$_ ~~ s/ \x5B .*//; # [
		$_ ~~ s/ \x7B .*//; # {
		$_ ~~ s/ \x5C .* //; # \
...
sub punctuation {
	...
	for @tracks -> $title { 
		...
		# Repeat for the other symbols
		# using ascii literal for each global char removal
		$_ ~~ s:g/\xBF //; # ¿
		$_ ~~ s:g/\xA1 //; # ¡
		$_ ~~ s:g/\x2E //; # .
		$_ ~~ s:g/\x21 //; # !
...
sub clean {
	...
	for @tracks -> $title {
		...
		##########################
		##                      ##
		# filtering out non-English characters, then skipping titles
		if $_ ~~ /<-[A..Za..z0..9\'\s]>/ { 
			next;  
        }
		##                      ##
		...
		# filtering out apostrophe or 's' or both	
		if $_ ~~ /^<[\s']>*$/ { 	
			next;
       	}
		##                      ##
...
sub build_bigrams {
	...
	for @tracks -> $title {
		# spitting titles into individual words
        my @words = $title.split(/\s+/);
        # checking if bigram (pairs of words)
        next if @words.elems < 2;
        # looping through array of words (stopping at boundary)
        for ^(@words.elems - 1) -> $i {
			# grabbing first word
            my $first_word = @words[$i];
			# grabbing firt word + 1 (second word in bigram)
            my $second_word = @words[$i + 1];
			# adding to our count of bigram pairs (in a hash of hashes format as per instr.)
            %counts{$first_word}{$second_word}++;     
        }
	}
...
sub mcw {
	...
	my $best_word = '';
	# initiliazing count for mcw
	my $best_count  = 0;

	# returning '' if no best words are found for input
    return '' unless %counts{$word}:exists;
    # finding most common "next word" for input
    for %counts{$word}.keys.sort -> $next_word {
        # jumping if word has already been counted
        next if %word_history{$next_word}:exists;
		# storing num of appearences
        my $bi_count = %counts{$word}{$next_word};
        # ensuring first max "wins" (as per instr.)
...
sub sequence {
	...
	##########################
	##                      ## 
	# keepin track of seed to build sequence
	my $seed = @_[0];
	# setting our seed as title
    my @title = $seed;
	# increasing history with each seed appeareance
    %word_history{$seed} = 1;

	# looping until we reach the end of our array of titles
    while @title.elems < $SEQUENCE_LENGTH {
		# finding the next best word based on input (in last part of arrayl *-1)
        my $next_word = mcw(@title[*-1]);
		# stoping if there are no best second words for input
        last if !$next_word;
		# adding next word to title and pushing
        @title.push: $next_word;
		# marking second word as used in out history
