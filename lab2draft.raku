...
sub build_bigrams {
	...
	for @tracks -> $title {
        ...
        # we need at least 2 words for a bigram
        next if @words.elems < 2;

        # slide a window of size‑2 across the title
        for ^(@words.elems - 1) -> $i {
            my $w1 = @words[$i];
            my $w2 = @words[$i + 1];
            %counts{$w1}{$w2}++;     # auto‑vivify & increment
        }
	}
...
sub mcw {
	...
	# if we have no outgoing edges for this word → return ''
    return '' unless %counts{$word}:exists;

    # get all candidate followers, sort for deterministic tie‑breaking
    for %counts{$word}.keys.sort -> $cand {

        # skip words that already appeared in current sequence
        next if %word_history{$cand}:exists;

        my $cnt = %counts{$word}{$cand};

        # strictly '>' so first max wins (after alphabetical sort)
        if $cnt > $best_count {
            $best_word  = $cand;
            $best_count = $cnt;
        }
...
sub sequence {
    ...
    my @title = $seed;           # start with the seed
    %word_history{$seed} = 1;
	while @title.elems < $SEQUENCE_LENGTH {
        my $next = mcw(@title[*-1]);
        last if !$next;          # no unseen follower → stop
        @title.push: $next;
        %word_history{$next} = 1;
    }
    return @title.join('');
